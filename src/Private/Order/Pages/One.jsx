...
export const One = () => {
  ...
  const onSubmit = (event) => {
    mutation.mutate({
      ...event,
      avtor: `${user.name} ${user.lastname}`,
      tovar,
      user_id: user.id,
      date: `${date.toLocaleDateString()}-${date.getHours()}:${date.getMinutes()} Harid qilindi`,
    });
  };
  watch();
  return (
    <>
      <div
        className="tovar_orders"
        style={{
          display: "grid",
          gridTemplate: "repeat(1, auto) / repeat(2, 50%)",
        ...
        <div style={{ background: "#fff" }} className="order_user">
          {tovar?.map((item) => (
            <>
              <h1>{item?.avtor}</h1>
              <div>
                <A variant="green" href={"tel:" + item?.telnumber}>
                  Telefon qilish
                </A>
              </div>
              <form onSubmit={handleSubmit(onSubmit)}>
                <label htmlFor="card_number">
                  <p className={errors?.card_number ? "error_text" : false}>
                    <small>
                      {errors?.card_number
                ...
                <Btn variant="yellow" type="submit">Yuborish</Btn>
              </form>
            </>
          ))}
        </div>
